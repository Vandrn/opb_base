...
//SELECT concat('$PREG_',lpad(ID_PREGUNTA,2,0),' = $_POST["','PREG_',lpad(ID_PREGUNTA,2,0),'"];') VAR FROM `dim_preguntas` where id_area = 6 and incluir = 1;
$PREG_47 = $_POST['PREG_47'] ?? null;
$PREG_48 = $_POST['PREG_48'] ?? null;
$PREG_49 = $_POST['PREG_49'] ?? null;
$OBS_06 = $_POST['OBS_06'] ?? null;

if (is_null($bv_pais_tienda_get)) {
    // Prepare and bind statement
    //SELECT concat('PREG_',lpad(ID_PREGUNTA,2,0),' = ?') VAR FROM `dim_preguntas` where id_area = 6 and incluir = 1;
    //SELECT concat('$PREG_',lpad(ID_PREGUNTA,2,0)) VAR FROM `dim_preguntas` where id_area = 6 and incluir = 1;
    $stmt = $conn2->prepare("UPDATE fact_visitas SET PREG_47 = ?,
PREG_48 = ?,
PREG_49 = ?, OBS_06 = ?
WHERE ID_VISITA = ?");
    $stmt->bind_param("ssssi", $PREG_47, $PREG_48, $PREG_49, $OBS_06, $id_registered_visit);

    // Execute statement
    $success = $stmt->execute();

    if ($success) {
        $insertMessage = "Información de Evaluación: Operaciones / Ventas ingresada con éxito.";
    } else {
    ...
    <div class="container">
        <form id="form1" action="step9.php" method="post">
            ...
            <div class="intro-text">
                <H3>SECCIÓN H - Evaluación: ¡ADOCKERS A BORDO!</H3>
                <?php
                ...
                <div>
                    ...
                    <input type="button" class="back-button" onclick="redirigirPagina('step7.php?id_registered_visit=<?php echo $id_registered_visit; ?>&format=<?php echo $format; ?>&bv_pais_tienda=<?php echo $bv_pais_tienda; ?>')" value="Regresar" />
                </div>
        </form>
    </div>
    <script>
